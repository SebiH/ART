...
#define PERF_OUTPUT(MESSAGE,TIMEPOINT1,TIMEPOINT2) \
	Log(std::string(MESSAGE) + std::to_string(std::chrono::duration_cast<std::chrono::microseconds>((__##TIMEPOINT2) - (__##TIMEPOINT1)).count())); \
...
extern "C" __declspec(dllexport) void Benchmark(int loglevel)
{
	szLogLevel = loglevel;

	{
		PERF_MEASURE(xml_start)
			// form XML document
			TiXmlDocument doc;
		...
		for (int i = 0; i < 10; i++)
		{
			...
			for (int i = 0; i < 10; i++)
			{
				...
				bone->SetAttribute("ID", 1);
				bone->SetAttribute("Name", "Test");
				bone->SetDoubleAttribute("x", 1.4);
				bone->SetDoubleAttribute("y", 1.4);
				bone->SetDoubleAttribute("z", 1.4);
				bone->SetDoubleAttribute("qx", 1.4);
				bone->SetDoubleAttribute("qy", 1.4);
				bone->SetDoubleAttribute("qz", 1.4);
				bone->SetDoubleAttribute("qw", 1.4);
				...
				for (auto j = 0; j < 5; j++)
				{
					...
					marker->SetDoubleAttribute("x", 1.4);
					marker->SetDoubleAttribute("y", 1.4);
					marker->SetDoubleAttribute("z", 1.4);
				}
			}
		...
		for (int i = 0; i < 10; i++)
		{
			...
			rb->SetAttribute("ID", 1);
			rb->SetAttribute("Name", "test");
			rb->SetDoubleAttribute("x", 1.4);
			rb->SetDoubleAttribute("y", 1.4);
			rb->SetDoubleAttribute("z", 1.4);
			rb->SetDoubleAttribute("qx", 1.4);
			rb->SetDoubleAttribute("qy", 1.4);
			rb->SetDoubleAttribute("qz", 1.4);
			rb->SetDoubleAttribute("qw", 1.4);
		...
		PERF_MEASURE(xml_end)
			PERF_OUTPUT("XML: ", xml_start, xml_end)
			Log("\n");

		Log(std::string("XML Length: ") + std::to_string(str.length()));
		Log("\n");
	}
...

	{
		...
		PERF_MEASURE(json_end)
			PERF_OUTPUT("JSON: ", json_start, json_end)
			Log("\n");
		Log(std::string("JSON Length: ") + std::to_string(str.length()));
		Log("\n");
