...
bool OptitrackInput::Start()
{
	...
	{
		is_running_ = true;
		Log::Info(Log::Format("Starting Optitrack Listener listening on %s for multicast from %s", LocalIp.c_str(), OptitrackIp.c_str()));
		CreateClient();
		InitClient();
		return true;
	}
...
void OptitrackInput::CreateClient()
{
	...
	Log::Info(Log::Format("Local IP:%s", LocalIp.c_str()));
	Log::Info(Log::Format("Optitrack IP:%s", OptitrackIp.c_str()));
	Log::Info(Log::Format("Server Name:%s", srv_description.szHostComputerName));
	Log::Info("");
}
...
void OptitrackInput::InitClient()
{
	...
	{
		Log::Info(Log::Format("Received: %s", (char*)response));
	}
	...
	Log::Info(Log::Format("Received %d Data Descriptions:", data_refs->nDataDescriptions));

	for (int i = 0; i < data_refs->nDataDescriptions; i++)
	{
		Log::Debug(Log::Format("Data Description # %d (type=%d)", i, data_refs->arrDataDescriptions[i].type));
		...
		if (data_refs->arrDataDescriptions[i].type == Descriptor_MarkerSet)
		{
			...
			Log::Debug(Log::Format("MarkerSet Name : %s", pMS->szName));
			for (int i = 0; i < pMS->nMarkers; i++)
			{
				Log::Debug(Log::Format("%s", pMS->szMarkerNames[i]));
			}
		...
		else if (data_refs->arrDataDescriptions[i].type == Descriptor_RigidBody)
		{
			...
			Log::Info(Log::Format("RigidBody Name : %s", pRB->szName));
			Log::Debug(Log::Format("RigidBody ID : %d", pRB->ID));
			Log::Debug(Log::Format("RigidBody Parent ID : %d", pRB->parentID));
			Log::Debug(Log::Format("Parent Offset : %3.2f,%3.2f,%3.2f", pRB->offsetx, pRB->offsety, pRB->offsetz));
		...
		else if (data_refs->arrDataDescriptions[i].type == Descriptor_Skeleton)
		{
			...
			Log::Info(Log::Format( "Skeleton Name : %s", pSK->szName));
			Log::Debug(Log::Format("Skeleton ID : %d", pSK->skeletonID));
			Log::Debug(Log::Format("RigidBody (Bone) Count : %d", pSK->nRigidBodies));
...
{
	Log::Debug(Log::Format("Received frame %d", motion_data->iFrame));
...
{
	Log::Warning(Log::Format("Error %d: %s", msg_type, msg));
}
