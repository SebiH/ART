...
extern "C" UNITY_INTERFACE_EXPORT int AddArToolkitStereoProcessor(const int pipeline_id, const char *json_config)
{
	try
	{
		auto pipeline = PipelineManager::Instance()->GetPipeline(pipeline_id);
		std::shared_ptr<Processor> artoolkit_processor = std::make_shared<ArToolkitStereoProcessor>(std::string(json_config));
		pipeline->AddProcessor(artoolkit_processor);
		return artoolkit_processor->Id();
	}
	catch (const std::exception &e)
	{
		DebugLog(std::string("Unable to add ARToolkitStereoProcessor: ") + e.what());
	}
