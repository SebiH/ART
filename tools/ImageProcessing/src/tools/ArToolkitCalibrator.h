...
namespace ImageProcessing
{
	...
	private:
		...
		double SingleCameraCalibration(const std::shared_ptr<CameraSourceInterface> &camera, const std::string &filename, const std::vector<std::vector<cv::Point2f>> &image_points, const cv::Size &image_size);
