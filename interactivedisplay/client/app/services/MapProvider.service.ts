    ...
    public initMaps() {
        ...
        for (let i = 0; i < mapCountHor; i++) {
            for (let j = 0; j < mapCountVer; j++) {
                let map = new Map();
                map.id = currentId++;
                map.posX = Math.max(i / (mapCountHor - 1) * width - mapSizeX/2, 0) + borderSize / 2;
                map.posY = Math.max(j / (mapCountVer - 1) * height - mapSizeY/2, 0) + borderSize / 2;
                map.sizeX = mapSizeX / 2 - borderSize;
