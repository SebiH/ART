...
std::vector<ProcessingOutput> RawImageModule::processImage(unsigned char *rawDataLeft, unsigned char *rawDataRight)
{
	...
	outputLeft.img = cv::Mat(cv::Size(_imgWidth, _imgHeight), CV_8UC3, outputLeft.data.get());
	...
	outputRight.img = cv::Mat(cv::Size(_imgWidth, _imgHeight), CV_8UC3, outputRight.data.get());
